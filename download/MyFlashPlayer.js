...
(function(global) {
	...
	;
	function setVideoId() {
		if(typeof(window.flashvarsObj) != 'undefined'
			&& typeof(window.flashvarsObj.uepflag)!= 'undefined') {
			document.getElementById('__flash2mplayer').setAttribute('uepflag', window.flashvarsObj.uepflag);
		}
	...
	function getYouku(vcode) {
		var uepflag = document.getElementById('__flash2mplayer').getAttribute('uepflag');
		if(uepflag == 1) {
			var url = 'http://v.youku.com/player/getPlayList/VideoIDS/' + vcode + '/timezone/+08/version/5/source/out/Sc/2?password=&ran=9777&n=3';
		} else {
			var url = 'http://v.youku.com/player/getPlayList/VideoIDS/' + vcode + '/timezone/+08/version/5/source/video?n=3&ran=5231&ctype=10&password=&ev=1';
		}
